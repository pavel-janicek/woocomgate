    ...
    class Clvr_Woo_Comgate extends WC_Payment_Gateway{
        ...
        public function init_form_fields(){
            ...
            $this->form_fields = array(
                     ...
                     'evil_hack' => array(
                        'title' => __('EVIL HACK', 'woocomgate'),
                        'type' => 'checkbox',
                        'label' => __('Natvrdo posílat maily po dokončení objednávky?', 'woothepay'),
                        'default' => 'yes'
                                     )
                 );
         }
        ...
        public function process_payment($order_id){
            ...
            $cookie_name = "woocomgate_order_id";
      ...
      $order->update_status('on-hold', __( 'Očekáváme platbu přes comgate', 'woogopay' ));
			return array(
            'result' => 'success',
        ...
        public function check_response($data){
            ...
            if ($data['status'] == self::PAID){
                ...
                if($this->evil_hack == 'yes'){
                    $this->send_completed_mail($order_id);
                }
            }
        ...
        public function show_success(){
            ...
            if(isset($_COOKIE['woocomgate_order_id'])){
                $order_id = $_COOKIE['woocomgate_order_id'];
                $order = new WC_Order($order_id);
                if ($order){
                    wp_redirect($order->get_checkout_order_received_url());
                    exit;
        ...
        public function send_completed_mail($order_id){
